...
# [타깃 폴더] 바탕화면의 proof_test_safe 폴더로 지정
DESKTOP_PATH = os.path.join(os.path.expanduser("~"), "Desktop")
TARGET_DIR = os.path.join(DESKTOP_PATH, "proof_test_safe")
os.makedirs(TARGET_DIR, exist_ok=True)

# j-hope Proof 5대 세부 쿼리 보충 매니페스트
JHOPE_PROOF_TASKS = [
    {"category": "lucky_draw", "query": "BTS j-hope Proof lucky draw photocard scan", "target_n": 3, "limit_2n": 25},
    {"category": "soundwave_m2u", "query": "BTS j-hope Proof Soundwave M2U photocard", "target_n": 3, "limit_2n": 25},
    {"category": "weverse_pob", "query": "BTS j-hope Proof Weverse POB photocard official", "target_n": 3, "limit_2n": 25},
    {"category": "japan_fc", "query": "BTS j-hope Proof Japan FC official photocard", "target_n": 2, "limit_2n": 20},
    {"category": "collectors", "query": "BTS j-hope Proof Collector Edition photocard scan", "target_n": 2, "limit_2n": 20},
]
...
def load_existing_cards_with_meta(target_dir: str) -> list[dict]:
    existing_items = []
...
async def process_supplement_urls(session: aiohttp.ClientSession, urls: list[str], target_n: int):
    existing_items = load_existing_cards_with_meta(TARGET_DIR)
    candidates = []
    headers = {
    ...
    for idx, url in enumerate(urls, 1):
        try:
            ...
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                ...
                for ex in existing_items:
                    ...
                    if sim >= 0.78:
                        ...
                        if new_q_score > ex["quality_score"]:
                            print(f"  🆙 기존 도안 퀄리티업 교체! ({ex['file_name']}: {ex['quality_score']:.1f}점 ➡ {new_q_score:.1f}점)")
                ...
                print(f"  ✨ 새로운 누락 도안 발굴! ({w}x{h}, {new_q_score:.1f}점)")
    ...
    for idx, item in enumerate(final_new_selected[:target_n], start_idx):
        file_name = f"test_jhope_proof_{idx:02d}.webp"
        file_path = os.path.join(TARGET_DIR, file_name)
        item["pil_img"].save(file_path, "WEBP", quality=95)
        saved_count += 1
        print(f"  💾 [신규 추가] {file_name} ({item['size'][0]}x{item['size'][1]}, {item['quality_score']:.1f}점)")

    print(f"  🎯 세부 쿼리 완료 (추가: +{saved_count}장 / 현재 총합: {len(existing_items) + saved_count}장)\n")
...
async def main():
    async with async_playwright() as p:
        ...
        async with aiohttp.ClientSession() as http_session:
            print(f"========================================================")
            print(f"🔥 BTS j-hope Proof 5대 세부 쿼리 보충 수집 가동")
            print(f"📂 대상 경로: {TARGET_DIR}")
            print(f"========================================================")

            for task_idx, task in enumerate(JHOPE_PROOF_TASKS, 1):
                query = task["query"]
                limit_2n = task["limit_2n"]
                target_n = task["target_n"]

                print(f"[{task_idx}/{len(JHOPE_PROOF_TASKS)}] 🔎 세부 탐색: '{query}'")
                ...
                print(f"📌 후보 URL {len(collected_urls)}개 확보 -> 정제 시작")
                await process_supplement_urls(http_session, list(collected_urls), target_n)
                await asyncio.sleep(random.uniform(2.5, 4.0))

        await browser.close()
        print("🎉 j-hope Proof 전 카테고리 보충 수집 완료!")
