...
TARGET_QUERIES = [
    {
        "category": "proof",
        "keyword": "BTS RM Proof photocard official",
        "target_n": 4,
        "sample_limit_2n": 10
    },
    {
        "category": "indigo",
        "keyword": "BTS RM Indigo photocard official",
        "target_n": 4,
        "sample_limit_2n": 10
    },
    {
        "category": "lucky_draw",
        "keyword": "BTS RM Lucky Draw photocard official",
        "target_n": 6,
        "sample_limit_2n": 14
    }
]
...
def validate_photocard_image(image_bytes: bytes) -> tuple[bool, str, int, int]:
    try:
        ...
        # A. 세로 비율 검사 (1.3 ~ 1.7) -> K-pop 포카 정규 비율(약 1:1.5) 엄격 적용
        ratio = h / w
        if ratio < 1.3 or ratio > 1.75:
        ...
        if contours:
            ...
            # 만약 가장 큰 객체가 전체 화면의 15%~75% 사이라면 바닥에 올려놓고 찍은 사진으로 판정
        ...
        if skin_ratio > 0.08:
            return False, f"손가락/인증샷 감지 (테두리 피부비율: {skin_ratio:.2%})", w, h
    ...
    async with async_playwright() as p:
        ...
        async with aiohttp.ClientSession() as http_session:
            for item in TARGET_QUERIES:
                ...
                while len(collected_urls) < limit_2n and scroll_attempts < 6:
                    images = await page.query_selector_all('div[data-test-id="pin"] img')
